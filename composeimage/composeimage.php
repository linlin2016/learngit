...
class ComposeImage {
	...
	private $width = 468;
	private $height = 468;
	private $leftImgWidth = 312;
	private $smallImgWidth = 96;
	private $smallImgHeight = 96;
	private $smallBoxWidth = 110;
	private $smallBoxHeight = 110;
	...
	*/
	public function getComposeImage($leftImg,$rightImages,$isPrice=false){
		...
		$new_im->newImage($this->width, $this->height, 'white', 'jpg');
		//合并图片
		$new_im->compositeImage($image, \Imagick::COMPOSITE_OVER, ($this->leftImgWidth - $newX) / 2, ($this->height - $newY) / 2);
		...
		foreach ($rightImages as $key=>$val){
			...
			$boxImg->compositeImage($smallImg, $smallImg->getImageCompose(), 8, 8);
			...
			$rightImg->compositeImage($boxImg, $boxImg->getImageCompose(), 0, $marginTop);
